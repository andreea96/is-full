function [Mid,Did,Dva] = ISLAB_5C(A,B,C,nk,N,sigma,lambda, fig_nr) 
%
% ISLAB_4C   Module that estimates an ARX model by using either LSM, MVI or MVI partial. 
%
...
%
% Last upgrade: March 16, 2004 / January 22, 2012
%
...
disp('Choose the estimation method:');
disp(' - 1 : Least Square Method(MCMMP)');
disp(' - 2 : Instrumental Variable Method(MVI)');
disp(' - 3 : Cancel;');
method = input('Method Number: ', 's');
if (strcmp(method, '3'))
    return;
end
...
if(strcmp(method, '2'))
    disp(' - 21 : MVI - classic');
    disp(' - 22 : MVI - partially filtered');
    disp(' - 23 : MVI - totally filtered');
    method = input('Method Number: ', 's');
end
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   for nb=1:Nb
      if ((na>1) || (nb>1)) 
         ...
         switch (method)
             ...
             case '22' % MVI - partially filtered
                 
                 NN = struc(1:4, 1:4, nk);
                 V = arxstruc(Did, Dva, NN);
                 order = selstruc(V, 0);
                 
                 aux = arx(Did, order);
                 uf = filter (aux.B, aux.A, Did.u);
                 uf = uf./sqrt(ones(N,1)*sum(uf.*uf)/N) ;
                 Did.u(1:na) = uf(1:na);
                 Mid = iv4(Did,[na nb nk]) ;% Model estimation. 
                 
             case '23' % MVI - totally filterted
                 ...
                 aux = arx(Did, order);
                 Mid = iv(Did,[na nb nk],aux.A, aux.B) ;% Model estimation.
