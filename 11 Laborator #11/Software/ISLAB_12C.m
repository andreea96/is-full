function [F,D,M] = ISLAB_12C(mt,K0,T0,Tmax,Ts,U,lambda) 
%
...
%~~~~~~~~~~~~~~~~
% M_state = n4sid(D,1);
%FOR PEM estimation comment the above code and uncomment the below 2 lines
M_state = n4sid(D,1);
M_state = pem(D,M_state);
theta1 = M_state.A;
theta2 = M_state.B;
alfa = 1006;
beta = 0.025;
...
FIG = FIG+1 ;
 ...
 yNoiseState = D.y - yState;
 noiseStateData = iddata(D.u,yNoiseState,Ts);
 noiseStateModel = armax(noiseStateData, [2 2 1 1]);
 yExternalNoise = sim(noiseStateModel,[D.u]);
 
   subplot(211);
   plot(t,D.y,'-r',t,yState+yExternalNoise,'-b',t,D.y,'-r') ; 
   title(join(['12.1-d \alpha=', num2str(alfa), ' \beta=', num2str(beta)]));
   legend('measured data', 'estimated data+noise') ; 
   
   subplot(212);
   vy_state=D.y-yState-yExternalNoise;
   meanVy = mean(vy_state,'all');
   sigmay_state = sum(vy_state-meanVy);
   plot(t,vy_state,'-r') ; 
   title(join(['\lambda^2_y=', num2str(sigmay_state)]));
   legend('output error');
