function [a,b,lambda,magi,phii,mag,phi,f] = ISLAB_4I(at,bt,K,N,nr, fig_nr) 
    ...
    at = poly(a) ; 
     ...
     na=length(at); % order of Auto Regressive polinomial
     nb=length(bt); % order of eXogen control polinomial
     nk=1;          % number of null coefficients of eXogen control polinomial
    ...
    for p=1:nr
       z = iddata(y(:,p),u(:,p),Ts) ;
       theta=arx(z,[na,nb,nk]);
       a=[a;theta.a];
       b=[b;theta.b]; 
       e=pe(theta,z);
       lambda = [lambda ; theta.noisevariance] ;% Standard deviation.  
    % 
    ...
    figure(FIG),clf ; 
       ...
       subplot(211) ; 
          ...
          title(strcat('Estimating an ARX[', num2str(na-1), ',', num2str(nb-1), '] model ', ... 
    ...
    figure(FIG),clf ; 
       ...
       text(nr/10,a(4)+0.15*e,... 
            ['        True parameters: ' ... 
              sprintf('%9.4f',[at(2:na) bt(2:nb)])]) ; 
       text(nr/10,a(4)+0.05*e,... 
            ['Estimated parameters: ' ... 
              sprintf('%9.4f',[am(2:na) bm(2:nb)])]) ; 
    FIG = FIG+1 ;
    %
    % END
    %
