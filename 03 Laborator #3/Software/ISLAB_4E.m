function [a,b,lambda,magi,phii,mag,phi,f] = ISLAB_4E(at,bt,K,N,nr, fig_nr) 
...
for p=1:nr
    ...
    R = [re_y(2) reu(2)-ryu(2); ... 
         reu(2)-ryu(2) ru(1)] ;    % Main matrix. 
   
    r = [re_y(1)-ry(2); ryu(3)] ;   % Free vector. 
    r = R\r ; 				    % Coefficients. 
   
    r(1)=(ryu(2)*ryu(3)-ru(1)*ry(2)+ru(1)*rye(2)-ryu(3)*reu(2))/(re_y(2)*ru(1)-(reu(2)-ryu(2))^2);
    r(2)=(re_y(2)*ryu(3)-ry(2)*ryu(2)+ryu(2)*rye(2)-reu(2)*rye(2)+reu(2)*ry(2))/(re_y(2)*ru(1)-(reu(2)-ryu(2))^2);    
...
figure(FIG),clf ; 
   ...
   subplot(211) ; 
      ...
      title(['Estimating an OE[1] model ' ... 
